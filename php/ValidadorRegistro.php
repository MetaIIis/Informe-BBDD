...
class ValidadorRegistro{
	...
	private function validarPassword1($password){
		...
		if (!$this->variableIniciada($password)) {
			return "Ingrese su contras&ntilde;a antes de continuar";
		}

		if (strlen($password) < 6) {
			return "Por tu seguridad la contrase&ntilde;a debe contener al menos 6 caracteres";
		}

		if ($this->nombre===$password) {
			return "Por tu seguridad el nombre no debe ser igual a tu contrase&ntilde;a";
		}
	...
	private function validarPassword2($password1, $password2){
		...
		if (!$this->variableIniciada($password1)) {
			return "Ingrese primero su contrase&ntilde;a";
		}

		if (!$this->variableIniciada($password2)) {
			return "Repita su contrase&ntilde;a";
		}

		if ($password1!==$password2) {
			return "Las contrase&ntilde;as no son iguales";
		}
